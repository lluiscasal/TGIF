...
function least_engaged() {
    ...
    for (var i = 0; i < apidata.results[0].members.length; i++) {
        ...
        obj.num_missed_votes = apidata.results[0].members[i]["missed_votes"];
        obj.perc_missed_votes = apidata.results[0].members[i]["missed_votes_pct"];
        listData.push(obj);
    }
    // order by the lowest values (sorting array of objects by attribute)
    listData.sort(function (obj1, obj2) {
        return obj2.num_missed_votes - obj1.num_missed_votes;
    });
    // get only the 10% first members  
...
function most_engaged() {
    ...
    listData.sort(function (obj1, obj2) {
        return obj1.num_missed_votes - obj2.num_missed_votes;
    });
...
function createTable(tbody_id, data) {
    ...
    for (i = 0; i < data.length; i++) {
        ...
        var votes = data[i].num_missed_votes;
        var votes_perc = data[i].perc_missed_votes;
